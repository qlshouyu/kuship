...
**Goals:**
- 在 `docker/docker-compose.yaml` 中声明三个服务：`redis:8.0-alpine`、`mysql:9`、`rainbond-dev:v6.7.1-release`。
- 满足约束：MySQL root 密码 `123456`；rainbond 通过环境变量连接 compose 网络内的 `mysql:9` 服务；rainbond 镜像由 `standalone/` 目录的 Dockerfile 构建产出。
...
- compose 默认网络让服务通过服务名 `mysql` 互相寻址，符合用户"使用 mysql:9"的约束。
...
- **Risk**：MySQL 9 与 rainbond 历史版本的 caching_sha2_password 兼容性需要确认。
  → Mitigation：保留显式的 `MYSQL_ROOT_HOST=%`、`command: --default-authentication-plugin=mysql_native_password` 选项的扩展余地；初版直接使用 mysql:9 默认配置，遇到兼容性问题再加。
