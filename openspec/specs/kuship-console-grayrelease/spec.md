# kuship-console-grayrelease

## Purpose

应用级灰度发布的控制平面契约：状态机（ACTIVE → COMPLETED / CANCELLED）、4 个 OpenAPI v1 端点 + 1 个 console 端点、ApisixRoute 后端权重通过 RegionClient 委托给 rainbond-go core 的 `/api-gateway/v1/{tenant_name}/routes/http`。本 capability 由 `add-gray-release` change 引入；模板实例化（创建灰度 service group）的真实链路依赖未迁移的 `AppInstallService`，当前以 stub 形式占位，待 `migrate-console-app-install` change 落地。
