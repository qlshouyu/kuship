...
定义 standalone 镜像离线包的生成契约：从 k3s 官方权威清单出发，按构建机架构生成 `k3s-images-<arch>.tar.zst`，保证 standalone 容器启动时 containerd 加载的镜像版本与 k3s 二进制版本完全一致。覆盖：版本钉死的单一真相源（`standalone/k3s-version.env`）、`standalone/images-package.sh` 的镜像清单获取与归档生成契约、`standalone/Dockerfile` 与 `standalone_build.sh` 之间共享同一版本号的协同方式，以及失败模式与可观测性约束。本能力修复了离线包内镜像版本与 k3s 运行期期望偏差导致的 ImagePullBackOff 回归。
