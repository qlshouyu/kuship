...
定义 `standalone_build.sh` 作为 standalone 镜像构建编排入口的契约：参数解析、代理开关、离线包缓存复用、与 `standalone/images-package.sh` 及 `docker buildx build` 的协作顺序。下游 `images-package.sh` 与 Dockerfile 通过本契约约束的接口被调用，使国内开发者可一键开代理并复用已有离线包。
