# rainbond-business-image-bundle

## Purpose

定义 Rainbond 业务镜像离线包的生成与加载契约：以 `standalone/rainbond-images.env` 作为唯一真相源声明 `RAINBOND_VERSION` 与镜像清单，由 `standalone/business-images-package.sh` 按当前架构生成 `rainbond-images-<arch>.tar.zst`，并保证清单与 `reference/rainbond-chart` 渲染产物一致；容器启动时由 `standalone/entrypoint.sh` 自动检测并就位到 k3s agent images 目录，使 rainbond 业务 Pod 在断网或弱网环境下也能秒级就绪。覆盖：业务镜像版本钉死的单一真相源、chart 渲染校对、按构建机架构产出的归档、必备工具与依赖检查、容器启动期幂等加载，以及"离线包不入库"的仓库约束。本能力与 standalone-image-bundle 互补：后者只负责 k3s 系统镜像（coredns / metrics-server / klipper-helm 等），本能力专注于 rainbond 业务镜像（rbd-api / rbd-worker / rainbond-operator / minio / apisix 等），两者产物相互独立、可单独构建与挂载。
