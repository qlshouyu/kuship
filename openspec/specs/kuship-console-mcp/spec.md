...
MCP（Model Context Protocol，2024-11-05）服务器的控制平面契约：SSE 长连接（`GET /console/mcp/query/sse`）+ 消息推送（`POST /console/mcp/query/message?session_id=`）+ HTTP RPC 升级（`POST /console/mcp/query` 支持 Accept text/event-stream）三端点构成 LLM 客户端 ↔ kuship 集群的双工通道。JSON-RPC 协议层（5 method：initialize / notifications/initialized / tools/list / tools/call / ping）+ Tool registry 抽象（`McpTool` 接口 + Spring `@Component` 自动收集）让新增工具零修改协议层。本 capability 由 `add-mcp-sse` change 引入，落地 8 个 MVP tool 覆盖 user / region / team / app / component / pod / log 查询；剩余 285 个 rainbond Python 端 tool 留给后续 batch hardening。
