...
- 本地运行的 MySQL，包含 rainbond-console 的 `console` 库（开发凭据 `root/123456`）
  - **schema 由 rainbond-console (Django) 拥有**：在干净环境下，必须先在 rainbond-console 中跑 `python manage.py migrate` 一次创建全部表（含 `team_helm_release_source` 等）。kuship-console 启动时 `hibernate.ddl-auto=validate` 会校验所有 entity 对应的表存在并字段对齐，缺失则启动失败。
- （可选）Docker，用于容器化构建
