...
`docker/docker-compose.yaml` 一键拉起本地开发依赖栈：`redis:8.0-alpine`、`mysql:9`（root 密码 `123456`）以及 `rainbond-dev:v6.9.0-release`。其中 rainbond 镜像由 `standalone/Dockerfile` 构建产出，首次启动前请先运行 `./standalone_build.sh` 以构建镜像并准备好 `k3s-images-<arch>.tar.zst` 离线包。
